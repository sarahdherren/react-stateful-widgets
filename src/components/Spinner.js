...
export default function Spinner() {
  ...
  const toggleSpinner = () => {
    const hide = document.querySelector('#spinner');
    spinner === true ? setSpinner(spinner = false) : setSpinner(spinner = true)
    
    spinner === true ?  hide.style.display = '' : hide.style.display = 'none';
  };
  ...
  return (
    <div className='widget-spinner container'>
      ...
      <button id='toggleSpinner' onClick={toggleSpinner}>
        {spinner != true ? 'show spinner' : 'hide spinner'}
      </button>
    </div>
  );
}
